...
function handleItemClick(e) {
  ...
  if (e.target.classList.contains("edit")) {
    e.preventDefault();
    document.getElementById("item").value =
      e.target.parentElement.childNodes[1].textContent.trim();
    const submit = document.getElementById("submit");
    submit.value = "EDIT";
    editItem = e;
  }
...
function clearAllTasks() {
  ...
  //Hide sort button after the task list is cleared
  document.querySelector(".sort_btn").style.display = "none";
...
//Function to sort Task list according to Due Date
function sortByDueDate() {
  const taskList = JSON.parse(localStorage.getItem("tasks"));

  const sortedTaskList = taskList.sort(
    (a, b) =>
      Number(new Date(a.dueDate.split(" ")[2])) -
      Number(new Date(b.dueDate.split(" ")[2]))
  );
