...
function saveTasksToLocalStorage() {
  ...
  tasks.forEach((task) => {
    ...
    const createdAt = task.querySelector("#created-at").textContent.slice(8);
    const dueDate = task
...
function sortByDueDate(order) {
  ...
  if (order === "Early") {
    ...
    taskList.sort((a, b) => new Date(b.dueDate) - new Date(a.dueDate));
  }
...
window.onclick = function (event) {
  if (!event.target.matches(".sort_btn")) {
